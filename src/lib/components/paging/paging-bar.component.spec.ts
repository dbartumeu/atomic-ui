...
describe('Component: PagingBar', () => {
  ...
  it('should set [pageSize] dynamically', (done: DoneFn) => {
    ...
    let pagingComponent: DebugElement = fixture.debugElement.query(By.directive(AtPagingBarComponent));
  ...
  it('should set [initialPage] and instanciate the paging bar at that page', (done: DoneFn) => {
    ...
    fixture.detectChanges();
    fixture.whenStable().then(() => {
      let page: number = fixture.debugElement.query(By.directive(AtPagingBarComponent)).componentInstance.page;
      expect(page).toBe(3);
      done();
    });
  });
  ...
  it('should render first/last buttons and then remove them when setting [firstLast] to false', (done: DoneFn) => {
    ...
    fixture.detectChanges();
    fixture.whenStable().then(() => {
      expect(fixture.debugElement.query(By.css('.at-paging-bar-first-page'))).toBeTruthy();
      expect(fixture.debugElement.query(By.css('.at-paging-bar-last-page'))).toBeTruthy();

      component.firstLast = false;
      fixture.detectChanges();
      fixture.whenStable().then(() => {
        expect(fixture.debugElement.query(By.css('.at-paging-bar-first-page'))).toBeFalsy();
        expect(fixture.debugElement.query(By.css('.at-paging-bar-last-page'))).toBeFalsy();
        done();
      });
    });
  });
  ...
  it('should set [pageLinkCount] and then see buttons in markup and then change count', (done: DoneFn) => {
    ...
    fixture.detectChanges();
    fixture.whenStable().then(() => {
      expect(fixture.debugElement.queryAll(By.css('.at-paging-bar-link-page')).length).toBe(6);

      component.pageLinkCount = 4;
      component.pageSize = 50;
      fixture.detectChanges();
      fixture.whenStable().then(() => {
        expect(fixture.debugElement.queryAll(By.css('.at-paging-bar-link-page')).length).toBe(4);
        done();
      });
    });
  });
  ...
  it('should navigate to page link 5 and then 3', (done: DoneFn) => {
    ...
    fixture.whenStable().then(() => {
      ...
      fixture.debugElement.queryAll(By.css('.at-paging-bar-link-page'))[4].nativeElement.click();
      fixture.detectChanges();
      fixture.whenStable().then(() => {
        expect(pagingComponent.componentInstance.page).toBe(5);
        fixture.debugElement.queryAll(By.css('.at-paging-bar-link-page'))[0].nativeElement.click();
        fixture.detectChanges();
        fixture.whenStable().then(() => {
...
@Component({
  template: `
    <at-paging-bar [pageSize]="pageSize" [total]="9215">
    </at-paging-bar>`,
})
...
@Component({
  template: `
    <at-paging-bar [initialPage]="initialPage" [total]="9215"></at-paging-bar>`,
})
...
@Component({
  template: `
    <at-paging-bar [firstLast]="firstLast" [total]="9333"></at-paging-bar>`,
})
...
@Component({
  template: `
    <at-paging-bar [pageLinkCount]="pageLinkCount" [pageSize]="pageSize" [total]="1345"></at-paging-bar>`,
})
...
@Component({
  template: `
    <at-paging-bar #pagingBar [pageSize]="100" [total]="650">
      <p>Go to:</p>
      <mat-form-field>
        <input #goToInput
                matInput
