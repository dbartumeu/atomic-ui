...
import {AtMediaService} from './at-media.service';

@Directive({
    selector: '[atMediaToggle]',
})
export class AtMediaToggleDirective implements OnInit, OnDestroy {
    ...
    set query(query: string) {
        if (!query) {
            throw new Error('Query needed for [atMediaToggle] directive.');
        }
        this._query = query;
    }
     ...
     * @param _mediaService
     */
    constructor(private _renderer: Renderer2, private _elementRef: ElementRef, private _mediaService: AtMediaService) {
    }
