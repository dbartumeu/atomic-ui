## AtFileUploadComponent: at-file-upload

## Usage

Perfect component for file selection and upload in simple flows. Uses `AtFileInputComponent` internally.

Example for usage:

```html
<at-file-upload #fileUpload [(ngModel)]="files" defaultColor="accent" activeColor="warn" cancelColor="primary" (select)="selectEvent($event)"
...
```typescript
import { AtFileService, IUploadOptions } from '@covalent/core';
...
  providers: [ AtFileService ]
})
export class Demo {

  file: File;
  
  constructor(private fileUploadService: AtFileService){ 
  };
